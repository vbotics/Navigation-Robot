...
function colorValue = blue_colorDetection(currFrame, objFlag,bnw)
colorValue=struct('color','no','blueObjectsMask',bnw);
eightBit= true;
if(objFlag==1)
...
% Assign the low and high thresholds for each color band.
		...
		blueThresholdLow = graythresh(blueBand);
        ...
        if eightBit
			...
			blueThresholdLow = uint8(blueThresholdLow * 255);
        end
        %blueThresholdLow;
...
% filter out small objects.
    ...
    blueObjectsMask = uint8(bwareaopen(blueObjectsMask, smallestAcceptableArea));
% 	subplot(3, 3, 5);
% 	imshow(blueObjectsMask, []);
...
% Fill in any holes in the regions, since they are most likely red also.
	blueObjectsMask = uint8(imfill(blueObjectsMask, 'holes'));
	subplot(3, 3, 7);
 	imshow(blueObjectsMask, []);
% 	title('Regions Filled');

% Save the binary image in the root folder
     blueObjectsMask = uint8( blueObjectsMask ) * 255;
     if(objFlag==1)
     imwrite( blueObjectsMask, 'blue.jpg');
     end
...
% Measure the mean RGB and area of all the detected blobs.
	[meanRGB, areas, numberOfBlobs] = MeasureBlobs(blueObjectsMask, redBand, greenBand, blueBand);
	if numberOfBlobs > 0
% 		fprintf(1, '\n----------------------------------------------\n');
...
% 		end
        ...
        colorValue.blueObjectsMask = blueObjectsMask;
	else
% 		% Alert user that no red blobs were found.
% 		message = sprintf('No blue blobs were found in the image:\n%s', fullImageFileName);
% 		fprintf(1, '\n%s\n', message);
